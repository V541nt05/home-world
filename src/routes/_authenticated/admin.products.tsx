...
function AdminProducts() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (form.image_url) {
      await supabase
        .from("product_images")
        .insert({ product_id: res.data.id, image_url: form.image_url, is_primary: true });
    }
    ...
    <div className="space-y-4">
      ...
      {form && (
        <form onSubmit={save} className="grid gap-3 rounded-md border bg-card p-4 sm:grid-cols-2">
          ...
          <input type="number" placeholder="Discount %" value={form.discount_percent} onChange={(e) => setForm({ ...form, discount_percent: e.target.value })} className="rounded-md border px-3 py-2 text-sm" />
          ...
          <input placeholder="Image URL" value={form.image_url} onChange={(e) => setForm({ ...form, image_url: e.target.value })} className="rounded-md border px-3 py-2 text-sm" />
          ...
          <div className="flex gap-2 sm:col-span-2">
            ...
            <button type="button" onClick={() => setForm(null)} className="rounded-md border px-4 py-2 text-sm">Cancel</button>
          </div>
        </form>
      )}
